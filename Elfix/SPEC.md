...
- **~~Open question:~~ CLOSED — the question was never *asked*, so it could not be answered.**
  `box_signature.py` was built and then imported by nothing: no consumer, no test, no export. With
  no downstream consumer there was no metric to gate "earns its keep" against, so "build it, then
  measure" stalled at *built*. Its marker sets (`{4,10}`, `{6,8,12,14}`, `{15}`) came from a 4×4
  diagram, not from data — the same species as the 3-6-9 axis assignment already dropped under
  Law 1. **Retired**, like converge points. A congruence signature (polynomial hash, earned base
  and modulus — `scripts/modulus_probe.py`) is the honest replacement *if a real identity-key need
  ever appears*; none currently does. Tier 1 is now `unit_point.py` alone: centre + width.
...
- **Open question:** which distance. Cosine in feature space, trajectory (DTW-style) distance, or an
  integer-lattice distance (feature directions chosen so distances are exact integers) — pick by
  held-out behaviour, not aesthetics. Box-signature overlap is no longer a candidate: the module is
  retired (Tier 1). The integer-lattice option is the only one that closes the "every value traces
  back to a count" claim on the *distances*, which today are floats no gate inspects.
